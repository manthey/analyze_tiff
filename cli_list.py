...
def processCLI(filename):
    ...
    if list_spec[cli]['type'] == 'python':
        if list_spec[cli].get('xml') and '--xml' in sys.argv[2:]:
            print(open(list_spec[cli]['xml']).read())
            return
        script_file = list_spec[cli].get('script', os.path.join(cli, os.path.basename(cli) + '.py'))
